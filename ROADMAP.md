...
- [x] Harden RC14 partner alerts with legacy-to-per-installation isolation, race-safe backfill and token reassignment, exact FCM-token claim binding, bounded rejected-token rotation, terminal invalid-token cleanup, immediate preference cleanup, and unpair purge coverage on disposable PostgreSQL.
...
- [x] Rotate machine-controlled database, session, token-pepper, TOTP-encryption, outbox-encryption, and push-encryption secrets; revoke existing sessions, one-use tokens, administrator MFA, and stored push addresses.
...
- [ ] Complete the two-phone, tablet, and Wear OS end-to-end RC14 release gate, including notifications, relationship purge, accessibility, large text, navigation modes, offline recovery, and smoke-account deletion.
- [ ] Promote the tested RC14 commit to 1.0 with version-only changes and repeat the critical artifact, device, and production checks.
