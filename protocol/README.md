...
1.3 adds the `/v4/quizzes` response family. A generated candidate declares `depth`, `theme_role`, and bounded `theme_tags`. A delivered day keeps the v2 privacy and typed-answer shape, the v3 caller-private feedback fields, and adds an optional public editorial theme. These labels contain no rating, review, account, couple, answer, or relationship data. The server may return empty theme tags or a null theme for legacy/fallback questions while preserving five stable questions.
...
- A v4 quiz day preserves that reveal rule. Theme and depth fields describe site-wide editorial composition and must never be interpreted as relationship scoring or private model output.
