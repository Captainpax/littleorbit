...
- Privileged administration uses Big Orbit under `/v2/admin`. Every fresh device starts with a ten-minute, five-attempt, hash-only terminal PIN plus password and P-256 key proof; the first owner then enrolls MFA, while existing MFA is preserved. Ordinary access requires password/MFA plus an approved key, single-use challenges, device-bound short sessions, per-device alerts, and allowlisted typed operations. The public web app and `/v1/admin` expose no admin console.
