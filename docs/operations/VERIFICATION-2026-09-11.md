...
- Python: Ruff, strict mypy, 35 pytest tests, Alembic head `0008`, and the ratcheted size/complexity checker.
...
- Public RC3 deployment: the GitHub release reports the same phone/Wear byte counts and SHA-256 digests as the locally verified artifacts; the migrated public API returns that exact optional metadata; `/download` renders the matching version, APK URL, and checksum; an API 36 debug client decoded and cached every trusted field; and version-code 1 requests continued to normal `401` authentication because no required-after time is active.
