...
- Immutable metadata was published at `2026-09-19T20:35:33.306439Z` with phone
  code 25, Wear code 19, compatibility floor 23, and no `required_after`
  enforcement time.
- The GitHub `v1.1.0` release is public rather than draft or prerelease. Its
  phone, Wear, and checksum assets have the exact generated byte counts.
- Production migration head remains `0026`. The API and web health checks pass,
  all long-running Compose services are up, and the post-deployment scan found
  zero API, worker, media-worker, web, or gateway error signatures.
- Local and public current-release responses agree on version, phone/Wear
  codes, compatibility floor, sizes, hashes, publication time, and inactive
  enforcement.
- Local and public complete phone and Wear downloads match all four expected
  byte counts and SHA-256 values. Both 1,024-byte public range requests returned
  `206` with exact total sizes.
- `/download`, `/patch-notes`, `/patch-notes.xml`, `/status`, `/showcase`, and
  local/public readiness returned `200`. RSS lists Little Orbit 1.1.0 first.
- Browser verification rendered the 1.1.0 download card, phone and Wear hashes,
  five release highlights, and 1.1.0-first patch notes without console errors.
- The public GitHub release is
  <https://github.com/Captainpax/littleorbit/releases/tag/v1.1.0>.
