...
Authentication loss is a separate account-wide boundary. An HTTP 401 from an authenticated repository, profile refresh, or display worker clears the encrypted session and all account and relationship caches. Public login and deletion-password proof calls use a separate execution path because their 401 responses do not prove that the bearer session itself is invalid. Home maps a failed refresh from the post-request session state, so an asynchronous failure cannot resurrect the stale signed-in screen after the purge.
...
- A revoked or expired Android session cannot retain thumbnails or present itself as a half-paired account.
