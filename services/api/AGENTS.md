...
- Create partner alerts in the accepted feature transaction, deliver independently per active installation, and acknowledge only the authenticated account's installation. Legacy account-wide state never suppresses a current delivery. Backfill is conflict-safe; preference disablement and unpairing discard waiting alerts. Serialize ownership changes for one FCM digest, bind each send result to the exact token digest and claim time, retain no reversible invalid token, and keep polling authoritative. Note-edit alerts use a 30-minute document/editor cooldown and are skipped while the partner already views the note.
