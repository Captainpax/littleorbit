...
- Fresh Big Orbit devices require a terminal-issued, hash-only eight-digit PIN with one active ten-minute capability and five-attempt invalidation. Consume it before issuing a restricted setup token, prove the pending Android Keystore key with the `bootstrap` signature domain, permit only bootstrap endpoints, and preserve existing MFA. Ordinary challenges expire and are consumed once; bind every short admin session to an approved non-revoked device and expose only typed jobs with no arbitrary command, SQL, path, URL, or argument.
