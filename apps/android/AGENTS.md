...
- Widgets and Wear surfaces read a minimal cache containing together-time, next countdown, update instant, opaque relationship identity, and no relationship content.
...
- Register only a random per-install UUID for partner alerts. Protect the local FCM address with Android Keystore and remove invalid or account-scoped values promptly. Quarantine a server-rejected address by digest, allow one bounded Firebase delete-and-rotate attempt until the server accepts an address, and keep polling active throughout. Fetch event metadata after a content-free WSS hint, coalesce work without cancelling an active fetch, acknowledge only after Android accepts the notification, and always use a generic public lock-screen version.
